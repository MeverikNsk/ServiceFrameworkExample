namespace Vsk.VooDoo.Core.Routing
{
    ...
    public class VooDooApplicationModelProvider : IApplicationModelProvider
    {
        ...
        public VooDooApplicationModelProvider(
        ...
        public void OnProvidersExecuted(ApplicationModelProviderContext context)
        {
            ...
            foreach (var controllerType in context.ControllerTypes)
            {
                ...
                var interfaceCheckCheckAvailability = interfaces.FirstOrDefault(c => c == typeof(IApiWithCheckAvailability));
                if (interfaceCheckCheckAvailability != null)
                {
                    try
                    {
                        if (_serviceProvider.GetRequiredService(controllerType) is IApiWithCheckAvailability controller && !controller.IsAvailability)
                        {
                            continue;
                        }
                    }
                ...
                foreach (var iface in interfaces)
                {
                    if (iface
                        ...
                        .Any())
                    {
                        var controllerModel = CreateControllerModel(iface.GetTypeInfo(), controllerType);
                        ...
                        foreach (var methodInfo in iface.GetMethods())
                        {
                            var actionModel = CreateActionModel(iface.GetTypeInfo(), controllerType, methodInfo);
                            if (actionModel == null)
                            {
                                continue;
                            }
        ...
        /// </summary>
        /// <param name="interfaceTypeInfo">The <see cref="TypeInfo"/>.</param>
        /// <returns>A <see cref="ControllerModel"/> for the given <see cref="TypeInfo"/>.</returns>
        internal static ControllerModel CreateControllerModel(TypeInfo interfaceTypeInfo, TypeInfo controllerTypeInfo)
        {
            ...
            var currentTypeInfo = interfaceTypeInfo;
            ...
            var attributes = interfaceTypeInfo.GetCustomAttributes(inherit: true);
            ...
            var controllerModel = new ControllerModel(controllerTypeInfo, attributes);

            AddRange(controllerModel.Selectors, CreateSelectors(attributes));

            controllerModel.ControllerName =
                controllerTypeInfo.Name.EndsWith("Controller", StringComparison.OrdinalIgnoreCase) ?
            ...
            if (typeof(IAsyncActionFilter).GetTypeInfo().IsAssignableFrom(interfaceTypeInfo) ||
                typeof(IActionFilter).GetTypeInfo().IsAssignableFrom(interfaceTypeInfo))
            {
                controllerModel.Filters.Add(new VooDooControllerActionFilter());
            }

            if (typeof(IAsyncResultFilter).GetTypeInfo().IsAssignableFrom(interfaceTypeInfo) ||
                typeof(IResultFilter).GetTypeInfo().IsAssignableFrom(interfaceTypeInfo))
            {
                controllerModel.Filters.Add(new VooDooControllerResultFilter());
            }
        ...
        /// </summary>
        /// <param name="interfaceTypeInfo">The interface <see cref="TypeInfo"/>.</param>
        /// <param name="controllerTypeInfo">The controller <see cref="TypeInfo"/>.</param>
            ...
            MethodInfo methodInfo)
        {
            ...
            if (!IsAction(interfaceTypeInfo, methodInfo))
            {
                return null;
            }
        ...
        private static IList<SelectorModel> CreateSelectors(IList<object> attributes)
        {
            ...
            // 2. [HttpPost]
            //
            // Note that having a route attribute that doesn't define a route template might be an error. We
